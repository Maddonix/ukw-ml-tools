from ..mongodb.validation import check_intervention_frames_extracted
from ..mongodb.update import update_many_frame_is_extracted
...
def update_frames_extracted(video_key, db_interventions, db_images):
    ...
    is_extracted_dict = check_intervention_frames_extracted(
        video_key, db_images, db_interventions
    )
    ids_extracted = [
        intervention.frames[i] for i, value in is_extracted_dict.items() if value
    ]
    ids_not_extracted = [
